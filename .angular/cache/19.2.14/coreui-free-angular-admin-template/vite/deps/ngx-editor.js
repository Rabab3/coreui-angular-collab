...
var Fragment = class _Fragment {
  ...
  */
  child(index) {
    ...
    if (!found3) throw new RangeError("Index " + index + " out of range for " + this);
    return found3;
  }
  /**
  ...
  */
  static resolveCached(doc4, pos) {
    for (let i = 0; i < resolveCache.length; i++) {
      ...
      if (cached.pos == pos && cached.doc == doc4) return cached;
    }
...
var Node = class _Node {
  ...
  */
  rangeHasMark(from2, to, type) {
    ...
    if (to > from2) this.nodesBetween(from2, to, (node) => {
      if (type.isInSet(node.marks)) found3 = true;
      return !found3;
    });
    return found3;
  }
  ...
  */
  check() {
    ...
    let copy2 = Mark.none;
    for (let i = 0; i < this.marks.length; i++) copy2 = this.marks[i].addToSet(copy2);
...
var Schema = class {
  ...
  */
  nodeType(name) {
    ...
    if (!found3) throw new RangeError("Unknown node type: " + name);
    return found3;
  }
};
  ...
  for (let i = 0; i < marks2.length; i++) {
    ...
    if (mark) {
      ...
      for (let prop in schema2.marks) {
        ...
        if (name == "_" || mark2.spec.group && mark2.spec.group.split(" ").indexOf(name) > -1) found3.push(ok = mark2);
      }
    }
...
var ParseContext = class {
  ...
  insertNode(node) {
    ...
    if (this.findPlace(node)) {
      ...
      for (let i = 0; i < node.marks.length; i++) if (!top.type || top.type.allowsMarkType(node.marks[i].type)) marks2 = node.marks[i].addToSet(marks2);
  ...
  findAtPoint(parent, offset2) {
    if (this.find) for (let i = 0; i < this.find.length; i++) {
      if (this.find[i].node == parent && this.find[i].offset == offset2) this.find[i].pos = this.currentPos;
    }
  }
  findInside(parent) {
    if (this.find) for (let i = 0; i < this.find.length; i++) {
  ...
  addPendingMark(mark) {
    let found3 = findSameMarkInSet(mark, this.top.pendingMarks);
    if (found3) this.top.stashMarks.push(found3);
    this.top.pendingMarks = mark.addToSet(this.top.pendingMarks);
  }
  removePendingMark(mark, upto) {
    for (let depth = this.open; depth >= 0; depth--) {
      let level = this.nodes[depth];
      let found3 = level.pendingMarks.lastIndexOf(mark);
      if (found3 > -1) {
  ...
  */
  serializeNodeInner(node, options) {
    let {
      dom,
      contentDOM
    } = _DOMSerializer.renderSpec(doc(options), this.nodes[node.type.name](node));
    if (contentDOM) {
  ...
  static renderSpec(doc4, structure, xmlNS = null) {
    ...
    for (let i = start; i < structure.length; i++) {
      ...
      if (child === 0) {
        ...
        } = _DOMSerializer.renderSpec(doc4, child, xmlNS);
...
// node_modules/prosemirror-transform/node_modules/prosemirror-model/dist/index.js
...
var Fragment2 = class _Fragment {
  ...
  */
  findIndex(pos, round2 = -1) {
    if (pos == 0) return retIndex2(0, pos);
    if (pos == this.size) return retIndex2(this.content.length, pos);
    if (pos > this.size || pos < 0) throw new RangeError(`Position ${pos} outside of fragment (${this})`);
    for (let i = 0, curPos = 0; ; i++) {
      let cur = this.child(i), end = curPos + cur.nodeSize;
      if (end >= pos) {
        if (end == pos || round2 > 0) return retIndex2(i + 1, end);
...
function retIndex2(index, offset2) {
  found2.index = index;
  found2.offset = offset2;
  return found2;
}
function compareDeep2(a, b) {
  ...
  if (array) {
    if (a.length != b.length) return false;
    for (let i = 0; i < a.length; i++) if (!compareDeep2(a[i], b[i])) return false;
  } else {
    for (let p in a) if (!(p in b) || !compareDeep2(a[p], b[p])) return false;
    for (let p in b) if (!(p in a)) return false;
  }
  return true;
}
...
var Slice2 = class _Slice {
  ...
  */
  insertAt(pos, fragment) {
    let content = insertInto2(this.content, pos + this.openStart, fragment);
    return content && new _Slice(content, this.openStart, this.openEnd);
  }
  /**
  @internal
  ...
  */
  static fromJSON(schema2, json) {
    ...
    let openStart = json.openStart || 0, openEnd = json.openEnd || 0;
    if (typeof openStart != "number" || typeof openEnd != "number") throw new RangeError("Invalid input for Slice.fromJSON");
    return new _Slice(Fragment2.fromJSON(schema2, json.content), openStart, openEnd);
  }
  /**
  Create a slice from a fragment by taking the maximum possible
  open value on both side of the fragment.
...
function removeRange2(content, from2, to) {
  ...
  if (index != indexTo) throw new RangeError("Removing non-flat range");
  return content.replaceChild(index, child.copy(removeRange2(child.content, from2 - offset2 - 1, to - offset2 - 1)));
}
function insertInto2(content, dist, insert, parent) {
  let {
    index,
    offset: offset2
  } = content.findIndex(dist), child = content.maybeChild(index);
  ...
  let inner = insertInto2(child.content, dist - offset2 - 1, insert);
  return inner && content.replaceChild(index, child.copy(inner));
}
function replace2($from, $to, slice2) {
  if (slice2.openStart > $from.depth) throw new ReplaceError2("Inserted content deeper than insertion position");
  if ($from.depth - slice2.openStart != $to.depth - slice2.openEnd) throw new ReplaceError2("Inconsistent open depths");
...
function replaceOuter2($from, $to, slice2, depth) {
  ...
  if (index == $to.index(depth) && depth < $from.depth - slice2.openStart) {
    ...
    } = prepareSliceForReplace2(slice2, $from);
    return close2(node, replaceThreeWay2($from, start, end, $to, depth));
  }
}
function checkJoin2(main, sub) {
  if (!sub.type.compatibleContent(main.type)) throw new ReplaceError2("Cannot join " + sub.type.name + " onto " + main.type.name);
}
function joinable2($before, $after, depth) {
  let node = $before.node(depth);
...
function addRange2($start, $end, depth, target) {
  ...
  for (let i = startIndex; i < endIndex; i++) addNode2(node.child(i), target);
  if ($end && $end.depth == depth && $end.textOffset) addNode2($end.nodeBefore, target);
}
...
function replaceThreeWay2($from, $start, $end, $to, depth) {
  ...
  if (openStart && openEnd && $start.index(depth) == $end.index(depth)) {
    ...
    if (openStart) addNode2(close2(openStart, replaceTwoWay2($from, $start, depth + 1)), content);
    addRange2($start, $end, depth, content);
    if (openEnd) addNode2(close2(openEnd, replaceTwoWay2($end, $to, depth + 1)), content);
  }
...
function prepareSliceForReplace2(slice2, $along) {
  let extra = $along.depth - slice2.openStart, parent = $along.node(extra);
  let node = parent.copy(slice2.content);
  for (let i = extra - 1; i >= 0; i--) node = $along.node(i).copy(Fragment2.from(node));
  return {
    start: node.resolveNoCache(slice2.openStart + extra),
    end: node.resolveNoCache(node.content.size - slice2.openEnd - extra)
  };
...
var ResolvedPos2 = class _ResolvedPos {
  ...
  */
  marks() {
    let parent = this.parent, index = this.index();
    if (parent.content.size == 0) return Mark2.none;
    if (this.textOffset) return parent.child(index).marks;
    let main = parent.maybeChild(index - 1), other = parent.maybeChild(index);
    if (!main) {
      let tmp = main;
  ...
  */
  blockRange(other = this, pred) {
    if (other.pos < this.pos) return other.blockRange(this);
    for (let d = this.depth - (this.parent.inlineContent || this.pos == other.pos ? 1 : 0); d >= 0; d--) if (other.pos <= this.end(d) && (!pred || pred(this.node(d)))) return new NodeRange2(this, other, d);
    return null;
  }
  /**
  Query whether the given position shares the same parent node.
  ...
  */
  static resolveCached(doc4, pos) {
    let cache = resolveCache2.get(doc4);
    if (cache) {
      ...
      resolveCache2.set(doc4, cache = new ResolveCache());
    }
...
var Node2 = class _Node {
  ...
  @internal
  */
  constructor(type, attrs, content, marks2 = Mark2.none) {
    this.type = type;
    this.attrs = attrs;
    this.marks = marks2;
  ...
  */
  hasMarkup(type, attrs, marks2) {
    return this.type == type && compareDeep2(this.attrs, attrs || type.defaultAttrs || emptyAttrs2) && Mark2.sameSet(this.marks, marks2 || Mark2.none);
  }
  /**
  Create a new node with the same markup as this node, containing
  the given content (or empty, if no content is given).
  ...
  */
  slice(from2, to = this.content.size, includeParents = false) {
    if (from2 == to) return Slice2.empty;
    let $from = this.resolve(from2), $to = this.resolve(to);
    let depth = includeParents ? 0 : $from.sharedDepth(to);
    let start = $from.start(depth), node = $from.node(depth);
    let content = node.content.cut($from.pos - start, $to.pos - start);
  ...
  */
  check() {
    ...
    let copy2 = Mark2.none;
    for (let i = 0; i < this.marks.length; i++) {
      let mark = this.marks[i];
      mark.type.checkAttrs(mark.attrs);
      copy2 = mark.addToSet(copy2);
    }
    if (!Mark2.sameSet(copy2, this.marks)) throw new RangeError(`Invalid collection of marks for node ${this.type.name}: ${this.marks.map((m) => m.type.name)}`);
    this.content.forEach((node) => node.check());
  }
  /**
  Return a JSON-serializeable representation of this node.
  ...
  */
  static fromJSON(schema2, json) {
    ...
    let content = Fragment2.fromJSON(schema2, json.content);
...
var ContentMatch2 = class _ContentMatch {
  ...
  */
  fillBefore(after, toEnd = false, startIndex = 0) {
    ...
    function search(match, types) {
      let finished = match.matchFragment(after, startIndex);
      if (finished && (!toEnd || finished.validEnd)) return Fragment2.from(types.map((tp) => tp.createAndFill()));
      for (let i = 0; i < match.next.length; i++) {
        let {
          type,
          next
...
function parseExprSubscript2(stream) {
  ...
  for (; ; ) {
    ...
    else if (stream.eat("{")) expr = parseExprRange2(stream, expr);
    else break;
  }
  return expr;
}
...
function parseExprRange2(stream, expr) {
  let min2 = parseNum2(stream), max2 = min2;
  if (stream.eat(",")) {
    if (stream.next != "}") max2 = parseNum2(stream);
    else max2 = -1;
  }
  if (!stream.eat("}")) stream.err("Unclosed braced range");
  return {
  ...
  function explore(states) {
    ...
    states.forEach((node) => {
      nfa3[node].forEach(({
        ...
        let set;
        for (let i = 0; i < out.length; i++) if (out[i][0] == term) set = out[i][1];
        nullFrom2(nfa3, to).forEach((node2) => {
          if (!set) out.push([term, set = []]);
          if (set.indexOf(node2) == -1) set.push(node2);
        });
      });
    });
    let state = labeled[states.join(",")] = new ContentMatch2(states.indexOf(nfa3.length - 1) > -1);
    for (let i = 0; i < out.length; i++) {
...
function initAttrs2(typeName, attrs) {
  let result = /* @__PURE__ */ Object.create(null);
  if (attrs) for (let name in attrs) result[name] = new Attribute2(typeName, name, attrs[name]);
  return result;
}
...
var MarkType2 = class _MarkType {
  ...
  */
  constructor(name, rank, schema2, spec) {
    ...
    this.schema = schema2;
    this.spec = spec;
    this.attrs = initAttrs2(name, spec.attrs);
    this.excluded = null;
    let defaults2 = defaultAttrs2(this.attrs);
    this.instance = defaults2 ? new Mark2(this, defaults2) : null;
  }
  /**
  Create a mark of this type. `attrs` may be `null` or an object
  containing only some of the mark's attributes. The others, if
  ...
  */
  create(attrs = null) {
    if (!attrs && this.instance) return this.instance;
    return new Mark2(this, computeAttrs2(this.attrs, attrs));
  }
  /**
  @internal
  */
  ...
  */
  static fromReplace(doc4, from2, to, slice2) {
    try {
      ...
      if (e instanceof ReplaceError2) return _StepResult.fail(e.message);
      throw e;
    }
  }
};
...
var AddNodeMarkStep = class _AddNodeMarkStep extends Step {
  ...
  */
  ...
  apply(doc4) {
    ...
    return StepResult.fromReplace(doc4, this.pos, this.pos + 1, new Slice2(Fragment2.from(updated), 0, node.isLeaf ? 0 : 1));
  }
  invert(doc4) {
    let node = doc4.nodeAt(this.pos);
    if (node) {
...
var ReplaceStep = class _ReplaceStep extends Step {
  ...
  */
  static fromJSON(schema2, json) {
    ...
    return new _ReplaceStep(json.from, json.to, Slice2.fromJSON(schema2, json.slice), !!json.structure);
  }
};
...
var ReplaceAroundStep = class _ReplaceAroundStep extends Step {
  ...
  */
  static fromJSON(schema2, json) {
    ...
    return new _ReplaceAroundStep(json.from, json.to, json.gapFrom, json.gapTo, Slice2.fromJSON(schema2, json.slice), json.insert, !!json.structure);
  }
};
  ...
  tr.doc.nodesBetween(from2, to, (node, pos) => {
    ...
    if (toRemove && toRemove.length) {
      ...
      for (let i = 0; i < toRemove.length; i++) {
        let style = toRemove[i], found3;
        for (let j = 0; j < matched.length; j++) {
          let m = matched[j];
          if (m.step == step - 1 && style.eq(matched[j].style)) found3 = m;
        }
        if (found3) {
          found3.to = end;
          found3.step = step;
    ...
    if (!allowed) {
      ...
      if (clearNewlines && child.isText && parentType.whitespace != "pre") {
        ...
        while (m = newline.exec(child.text)) {
          if (!slice2) slice2 = new Slice2(Fragment2.from(parentType.schema.text(" ", parentType.allowedMarks(child.marks))), 0, 0);
...
function wrap(tr, range, wrappers) {
  ...
  tr.step(new ReplaceAroundStep(start, end, start, end, new Slice2(content, 0, 0), wrappers.length, true));
}
function setBlockType(tr, from2, to, type, attrs) {
  ...
  tr.doc.nodesBetween(from2, to, (node, pos) => {
    ...
    if (node.isTextblock && !node.hasMarkup(type, attrsHere) && canChangeType(tr.doc, tr.mapping.slice(mapFrom).map(pos), type)) {
      ...
      tr.step(new ReplaceAroundStep(startM, endM, startM + 1, endM - 1, new Slice2(Fragment2.from(type.create(attrsHere, null, node.marks)), 0, 0), 1, true));
...
function setNodeMarkup(tr, pos, type, attrs, marks2) {
  ...
  tr.step(new ReplaceAroundStep(pos, pos + node.nodeSize, pos + 1, pos + node.nodeSize - 1, new Slice2(Fragment2.from(newNode), 0, 0), 1, true));
}
...
function canJoin(doc4, pos) {
  let $pos = doc4.resolve(pos), index = $pos.index();
  return joinable3($pos.nodeBefore, $pos.nodeAfter) && $pos.parent.canReplace(index, index + 1);
}
...
var Fitter = class {
  constructor($from, $to, unplaced) {
    ...
    for (let i = $from.depth; i > 0; i--) this.placed = Fragment2.from($from.node(i).copy(this.placed));
  }
  get depth() {
  ...
  fit() {
    ...
    let slice2 = new Slice2(content, openStart, openEnd);
    ...
    for (let pass = 1; pass <= 2; pass++) {
      for (let sliceDepth = pass == 1 ? startDepth : this.unplaced.openStart; sliceDepth >= 0; sliceDepth--) {
        ...
        for (let frontierDepth = this.depth; frontierDepth >= 0; frontierDepth--) {
          ...
          if (pass == 1 && (first ? match.matchType(first.type) || (inject = match.fillBefore(Fragment2.from(first), false)) : parent && type.compatibleContent(parent.type))) return {
            sliceDepth,
            frontierDepth,
            parent,
            inject
  ...
  openMore() {
    ...
    this.unplaced = new Slice2(content, openStart + 1, Math.max(openEnd, inner.size + openStart >= content.size - openEnd ? openStart + 1 : 0));
    return true;
  }
  dropNode() {
    let {
    ...
    if (inner.childCount <= 1 && openStart > 0) {
      ...
      this.unplaced = new Slice2(dropFromFragment(content, openStart - 1, 1), openStart - 1, openAtEnd ? openStart - 1 : openEnd);
    } else {
      this.unplaced = new Slice2(dropFromFragment(content, openStart, 1), openStart, openEnd);
    }
  }
  ...
  close($to) {
    ...
    if (!close3) return null;
    while (this.depth > close3.depth) this.closeFrontierNode();
    if (close3.fit.childCount) this.placed = addToFragment(this.placed, close3.depth, close3.fit);
    $to = close3.move;
...
function closeNodeStart(node, openStart, openEnd) {
  ...
  if (openStart > 0) {
    ...
    if (openEnd <= 0) frag = frag.append(node.type.contentMatch.matchFragment(frag).fillBefore(Fragment2.empty, true));
  }
...
function replaceRange(tr, from2, to, slice2) {
  ...
  for (let j = slice2.openStart; j >= 0; j--) {
    ...
    for (let i = 0; i < targetDepths.length; i++) {
      ...
      if (parent.canReplaceWith(index, index, insert.type, insert.marks)) return tr.replace($from.before(targetDepth), expand ? $to.after(targetDepth) : to, new Slice2(closeFragment(slice2.content, 0, slice2.openStart, openDepth), openDepth, slice2.openEnd));
    }
  }
...
function replaceRangeWith(tr, from2, to, node) {
  ...
  tr.replaceRange(from2, to, new Slice2(Fragment2.from(node), 0, 0));
}
...
var Transform = class {
  ...
  */
  replaceWith(from2, to, content) {
    return this.replace(from2, to, new Slice2(Fragment2.from(content), 0, 0));
  }
  /**
...
function scrollRectIntoView(view, rect, startDOM) {
  ...
  for (let parent = startDOM || view.dom; ; parent = parentNode(parent)) {
    ...
    else if (rect.bottom > bounding.bottom - getSide(scrollThreshold, "bottom")) moveY = rect.bottom - bounding.bottom + getSide(scrollMargin, "bottom");
    ...
    for (let i = 0; i < rects.length; i++) {
      ...
      if (rect.top <= rowBot && rect.bottom >= rowTop) {
        ...
        if (dx < dxClosest) {
          ...
          if (child.nodeType == 1 && dx) offset2 = childIndex + (coords.left >= (rect.left + rect.right) / 2 ? 1 : 0);
          continue;
        }
      ...
      if (!closest && (coords.left >= rect.right && coords.top >= rect.top || coords.left >= rect.left && coords.top >= rect.bottom)) offset2 = childIndex + 1;
    }
  }
  ...
  if (node) {
    if (gecko && node.nodeType == 1) {
      ...
      if (offset2 < node.childNodes.length) {
        ...
        if (next.nodeName == "IMG" && (box = next.getBoundingClientRect()).right <= coords.left && box.bottom > coords.top) offset2++;
      }
    }
    if (node == view.dom && offset2 == node.childNodes.length - 1 && node.lastChild.nodeType == 1 && coords.top > node.lastChild.getBoundingClientRect().bottom) pos = view.state.doc.content.size;
    else if (offset2 == 0 || node.nodeType != 1 || node.childNodes[offset2 - 1].nodeName != "BR") pos = posFromCaret(view, node, offset2, coords);
  }
...
var ViewDesc = class {
  ...
  localPosFromDOM(dom, offset2, bias) {
    ...
    if (dom == this.dom && this.contentDOM) {
      ...
      if (offset2 == 0) for (let search = dom; ; search = search.parentNode) {
        if (search == this.dom) {
          atEnd = false;
          break;
        }
      ...
      if (atEnd == null && offset2 == dom.childNodes.length) for (let search = dom; ; search = search.parentNode) {
        if (search == this.dom) {
          atEnd = true;
          break;
        }
  ...
  descAt(pos) {
    for (let i = 0, offset2 = 0; i < this.children.length; i++) {
      ...
      if (pos < end) return child.descAt(pos - offset2 - child.border);
      offset2 = end;
    }
  }
  domFromPos(pos, side) {
    ...
    if (offset2) return this.children[i].domFromPos(offset2 - this.children[i].border, side);
  ...
  domAfterPos(pos) {
    ...
    if (node.nodeType != 1 || offset2 == node.childNodes.length) throw new RangeError("No node after pos " + pos);
  ...
  setSelection(anchor, head, root, force = false) {
    ...
    for (let i = 0, offset2 = 0; i < this.children.length; i++) {
      let child = this.children[i], end = offset2 + child.size;
      if (from2 > offset2 && to < end) return child.setSelection(anchor - offset2 - child.border, head - offset2 - child.border, root, force);
      offset2 = end;
    }
...
var CompositionViewDesc = class extends ViewDesc {
  ...
  localPosFromDOM(dom, offset2) {
    if (dom != this.textDOM) return this.posAtStart + (offset2 ? this.size : 0);
...
var MarkViewDesc = class _MarkViewDesc extends ViewDesc {
  ...
  static create(parent, mark, inline2, view) {
    ...
    if (!spec || !spec.dom) spec = DOMSerializer.renderSpec(document, mark.type.spec.toDOM(mark, inline2));
...
var TextViewDesc = class _TextViewDesc extends NodeViewDesc {
  ...
  localPosFromDOM(dom, offset2, bias) {
    if (dom == this.nodeDOM) return this.posAtStart + Math.min(offset2, this.node.text.length);
...
function iterDeco(parent, deco, onWidget, onNode) {
  ...
  for (let parentIndex = 0; ; ) {
    if (decoIndex < locals.length && locals[decoIndex].to == offset2) {
      ...
      while (decoIndex < locals.length && locals[decoIndex].to == offset2) (widgets || (widgets = [widget])).push(locals[decoIndex++]);
      if (widgets) {
    ...
    for (let i = 0; i < active.length; i++) if (active[i].to <= offset2) active.splice(i--, 1);
    while (decoIndex < locals.length && locals[decoIndex].from <= offset2 && locals[decoIndex].to > offset2) active.push(locals[decoIndex++]);
...
function nearbyTextNode(node, offset2) {
  for (; ; ) {
    ...
    if (node.nodeType == 1 && offset2 > 0) {
      if (node.childNodes.length > offset2 && node.childNodes[offset2].nodeType == 3) return node.childNodes[offset2];
...
function findTextInFragment(frag, text2, from2, to) {
  for (let i = 0, pos = 0; i < frag.childCount && pos <= to; ) {
    ...
    if (pos >= from2) {
      ...
      if (found3 >= 0 && found3 + text2.length + childStart >= from2) return childStart + found3;
...
function skipIgnoredNodesBefore(view) {
  ...
  if (gecko && node.nodeType == 1 && offset2 < nodeLen(node) && isIgnorable(node.childNodes[offset2])) force = true;
  for (; ; ) {
    if (offset2 > 0) {
      if (node.nodeType != 1) {
        break;
  ...
  if (force) setSelFocus(view, node, offset2);
...
editHandlers.paste = (view, _event) => {
  ...
  let data = brokenClipboardAPI ? null : event.clipboardData;
  let plain = view.input.shiftKey && view.input.lastKeyCode != 45;
  if (data && doPaste(view, data.getData("text/plain"), data.getData("text/html"), plain, event)) event.preventDefault();
...
var DecorationSet = class _DecorationSet {
  ...
  */
  ...
  findInner(start, end, result, offset2, predicate) {
    for (let i = 0; i < this.local.length; i++) {
      ...
      if (span.from <= end && span.to >= start && (!predicate || predicate(span.spec))) result.push(span.copy(span.from + offset2, span.to + offset2));
    }
  ...
  */
  mapInner(mapping, node, offset2, oldOffset, options) {
    ...
    if (this.children.length) return mapChildren(this.children, newLocal || [], mapping, node, offset2, oldOffset, options);
  ...
  */
  ...
  addInner(doc4, decorations, offset2) {
    ...
    doc4.forEach((childNode, childOffset) => {
      let baseOffset = childOffset + offset2, found3;
      if (!(found3 = takeSpansForNode(decorations, childNode, baseOffset))) return;
      if (!children) children = this.children.slice();
      while (childIndex < children.length && children[childIndex] < childOffset) childIndex += 3;
      if (children[childIndex] == childOffset) children[childIndex + 2] = children[childIndex + 2].addInner(childNode, found3, baseOffset + 1);
      else children.splice(childIndex, 0, childOffset, childOffset + childNode.nodeSize, buildTree(found3, childNode, baseOffset + 1, noSpec));
      childIndex += 3;
    });
  ...
  removeInner(decorations, offset2) {
    ...
    if (local.length) {
      for (let i = 0, span; i < decorations.length; i++) if (span = decorations[i]) {
        for (let j = 0; j < local.length; j++) if (local[j].eq(span, offset2)) {
  ...
  */
  forChild(offset2, node) {
    ...
    let child, local;
    for (let i = 0; i < this.children.length; i += 3) if (this.children[i] >= offset2) {
...
var DecorationGroup = class _DecorationGroup {
  ...
  forChild(offset2, child) {
    ...
    for (let i = 0; i < this.members.length; i++) {
      ...
      if (result instanceof _DecorationGroup) found3 = found3.concat(result.members);
      else found3.push(result);
    }
...
function takeSpansForNode(spans, node, offset2) {
  ...
  let end = offset2 + node.nodeSize, found3 = null;
...
function buildTree(spans, node, offset2, options) {
  ...
  node.forEach((childNode, localStart) => {
    let found3 = takeSpansForNode(spans, childNode, localStart + offset2);
    if (found3) {
      hasNulls = true;
      let subtree = buildTree(found3, childNode, offset2 + localStart + 1, options);
...
function viewDecorations(view) {
  ...
  view.someProp("decorations", (f) => {
    ...
    if (result && result != empty) found3.push(result);
  });
  if (view.cursorWrapper) found3.push(DecorationSet.create(view.state.doc, [view.cursorWrapper.deco]));
...
function safariShadowSelectionRange(view) {
  ...
  let anchorNode = found3.startContainer, anchorOffset = found3.startOffset;
  let focusNode = found3.endContainer, focusOffset = found3.endOffset;
...
function readDOMChange(view, from2, to, typeOver, addedNodes) {
  if (from2 < 0) {
    ...
    if (newSel && !view.state.selection.eq(newSel)) {
      ...
      if (view.composing) tr2.setMeta("composition", view.input.compositionID);
      view.dispatch(tr2);
    }
    return;
  }
  ...
  if (chrome && view.cursorWrapper && parse.sel && parse.sel.anchor == view.cursorWrapper.deco.from && parse.sel.head == parse.sel.anchor) {
    ...
    parse.sel = {
      anchor: parse.sel.anchor + size2,
      head: parse.sel.anchor + size2
    };
  }
...
function doWrapInList(tr, range, wrappers, joinBefore, listType) {
  ...
  let found3 = 0;
  for (let i = 0; i < wrappers.length; i++) if (wrappers[i].type == listType) found3 = i + 1;
...
function toggleMark(markType, attrs = null) {
  return function(state, dispatch) {
    ...
    if (empty2 && !$cursor || !markApplies(state.doc, ranges, markType)) return false;
    if (dispatch) {
      if ($cursor) {
  ...
  return {
    ...
    fn(state) {
      return __async(this, null, function* () {
        ...
        const currentOverflows = [overflow[getSide2(currentPlacement)], overflow[main], overflow[cross]];
...
function isContainingBlock(element) {
  const safari2 = isSafari();
  const css = getComputedStyle$1(element);
  return css.transform !== "none" || css.perspective !== "none" || (css.containerType ? css.containerType !== "normal" : false) || !safari2 && (css.backdropFilter ? css.backdropFilter !== "none" : false) || !safari2 && (css.filter ? css.filter !== "none" : false) || ["transform", "perspective", "filter"].some((value) => (css.willChange || "").includes(value)) || ["paint", "layout", "strict", "content"].some((value) => (css.contain || "").includes(value));
}
function isSafari() {
...
function applyTransaction(history2, state, tr, options) {
  ...
  if (tr.steps.length == 0) {
    ...
    if (appended.getMeta(historyKey).redo) return new HistoryState(history2.done.addTransform(tr, void 0, options, mustPreserveItems(state)), history2.undone, rangesFor(tr.mapping.maps[tr.steps.length - 1]), history2.prevTime, history2.prevComposition);
...
function LinkComponent_div_3_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    ɵɵadvance(4);
    ɵɵtextInterpolate(ɵɵpipeBind1(6, 8, ctx_r1.getLabel("url")));
    ɵɵadvance(3);
    ɵɵproperty("ngIf", ctx_r1.href.touched && ctx_r1.href.invalid);
    ɵɵadvance(4);
    ɵɵtextInterpolate(ɵɵpipeBind1(13, 10, ctx_r1.getLabel("text")));
    ɵɵadvance(3);
    ɵɵproperty("ngIf", ctx_r1.text.touched && ctx_r1.text.invalid);
    ɵɵadvance(5);
    ɵɵtextInterpolate1(" ", ɵɵpipeBind1(21, 12, ctx_r1.getLabel("openInNewTab")), " ");
    ɵɵadvance(2);
    ɵɵproperty("disabled", !ctx_r1.form.valid);
    ɵɵadvance();
    ɵɵtextInterpolate(ɵɵpipeBind1(24, 14, ctx_r1.getLabel("insert")));
  }
}
var _c4 = (a0, a1) => ({
...
function DropdownComponent_div_3_div_1_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    const ctx_r2 = ɵɵnextContext(2);
    ɵɵproperty("ngClass", ɵɵpureFunction2(4, _c4, item_r2 === ctx_r2.activeItem, ctx_r2.disabledItems.includes(item_r2)));
    ɵɵadvance();
    ɵɵtextInterpolate1(" ", ɵɵpipeBind1(2, 2, ctx_r2.getName(item_r2)), " ");
  }
}
...
function ImageComponent_div_3_Template(rf, ctx) {
  ...
  if (rf & 2) {
    ...
    ɵɵproperty("formGroup", ctx_r1.form);
    ɵɵadvance(4);
    ɵɵtextInterpolate(ɵɵpipeBind1(6, 7, ctx_r1.getLabel("url")));
    ɵɵadvance(3);
    ɵɵproperty("ngIf", ctx_r1.src.touched && ctx_r1.src.invalid);
    ɵɵadvance(4);
    ɵɵtextInterpolate(ɵɵpipeBind1(13, 9, ctx_r1.getLabel("altText")));
    ɵɵadvance(6);
    ɵɵtextInterpolate(ɵɵpipeBind1(19, 11, ctx_r1.getLabel("title")));
    ɵɵadvance(3);
    ɵɵproperty("disabled", !ctx_r1.form.valid || !ctx_r1.form.dirty);
    ɵɵadvance();
    ɵɵtextInterpolate(ɵɵpipeBind1(23, 13, ctx_r1.getLabel("insert")));
  }
}
var _c5 = (a0, a1) => ({
  backgroundColor: a0,
...
function ColorPickerComponent_div_3_div_1_button_1_Template(rf, ctx) {
  if (rf & 1) {
    ...
    ɵɵlistener("mousedown", function ColorPickerComponent_div_3_div_1_button_1_Template_button_mousedown_0_listener($event) {
      ...
      return ɵɵresetView(ctx_r3.onColorSelect($event, color_r3));
    });
    ɵɵelementEnd();
  }
  if (rf & 2) {
...
function ColorPickerComponent_div_3_Template(rf, ctx) {
  if (rf & 1) {
    ...
    ɵɵlistener("mousedown", function ColorPickerComponent_div_3_Template_button_mousedown_2_listener($event) {
      ...
      return ɵɵresetView(ctx_r3.remove($event));
    });
    ɵɵtext(3);
    ɵɵpipe(4, "async");
    ɵɵelementEnd()();
...
var ImageViewComponent = class _ImageViewComponent {
  ...
  static {
    this.ɵfac = function ImageViewComponent_Factory(__ngFactoryType__) {
...
var NgxEditorComponent = class _NgxEditorComponent {
  ...
  static {
    this.ɵfac = function NgxEditorComponent_Factory(__ngFactoryType__) {
...
var STRONG = new Mark3("strong");
var EM = new Mark3("em");
var CODE = new Mark3("code");
var UNDERLINE = new Mark3("u");
var STRIKE = new Mark3("s");
...
var NgxEditorServiceConfig = class _NgxEditorServiceConfig {
  ...
  static {
    this.ɵfac = function NgxEditorServiceConfig_Factory(__ngFactoryType__) {
...
var NgxEditorService = class _NgxEditorService {
  ...
  static {
    this.ɵfac = function NgxEditorService_Factory(__ngFactoryType__) {
...
var ToggleCommandComponent = class _ToggleCommandComponent {
  ...
  static {
    this.ɵfac = function ToggleCommandComponent_Factory(__ngFactoryType__) {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 3,
      vars: 10,
      consts: [[1, "NgxEditor__MenuItem--IconContainer", 3, "mousedown", "innerHTML", "title"]],
...
var InsertCommandComponent = class _InsertCommandComponent {
  ...
  static {
    this.ɵfac = function InsertCommandComponent_Factory(__ngFactoryType__) {
...
var LinkComponent = class _LinkComponent {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 4,
      vars: 11,
      consts: [[1, "NgxEditor__MenuItem--IconContainer", 3, "mousedown", "innerHTML", "title"], ["class", "NgxEditor__Popup", 4, "ngIf"], [1, "NgxEditor__Popup"], [1, "NgxEditor__Popup--Form", 3, "ngSubmit", "formGroup"], [1, "NgxEditor__Popup--FormGroup"], [1, "NgxEditor__Popup--Col"], [1, "NgxEditor__Popup--Label"], ["type", "href", "id", "href", "formControlName", "href", "autofocus", "", "autocomplete", "off"], ["class", "NgxEditor__HelpText NgxEditor__HelpText--Error", 4, "ngIf"], ["type", "text", "formControlName", "text", "autocomplete", "off"], ["type", "checkbox", "formControlName", "openInNewTab"], ["type", "submit", 3, "disabled"], [1, "NgxEditor__HelpText", "NgxEditor__HelpText--Error"]],
...
var DropdownComponent = class _DropdownComponent {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 4,
      vars: 8,
      consts: [[1, "NgxEditor__Dropdown--Text", 3, "mousedown"], ["class", "NgxEditor__Dropdown--DropdownMenu", 4, "ngIf"], [1, "NgxEditor__Dropdown--DropdownMenu"], ["class", "NgxEditor__Dropdown--Item", 3, "ngClass", "mousedown", 4, "ngFor", "ngForOf", "ngForTrackBy"], [1, "NgxEditor__Dropdown--Item", 3, "mousedown", "ngClass"]],
...
var ImageComponent = class _ImageComponent {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 4,
      vars: 9,
      consts: [[1, "NgxEditor__MenuItem--IconContainer", 3, "mousedown", "innerHTML", "title"], ["class", "NgxEditor__Popup", 4, "ngIf"], [1, "NgxEditor__Popup"], [1, "NgxEditor__Popup--Form", 3, "ngSubmit", "formGroup"], [1, "NgxEditor__Popup--FormGroup"], [1, "NgxEditor__Popup--Col"], [1, "NgxEditor__Popup--Label"], ["type", "href", "id", "href", "formControlName", "src", "autofocus", "", "autocomplete", "off"], ["class", "NgxEditor__HelpText NgxEditor__HelpText--Error", 4, "ngIf"], ["type", "text", "formControlName", "alt", "autocomplete", "off"], ["type", "text", "formControlName", "title", "autocomplete", "off"], ["type", "submit", 3, "disabled"], [1, "NgxEditor__HelpText", "NgxEditor__HelpText--Error"]],
...
var ColorPickerComponent = class _ColorPickerComponent {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 4,
      vars: 11,
      consts: [[1, "NgxEditor__MenuItem--IconContainer", 3, "mousedown", "innerHTML", "title"], ["class", "NgxEditor__Popup", 4, "ngIf"], [1, "NgxEditor__Popup"], ["class", "NgxEditor__ColorContainer", 4, "ngFor", "ngForOf", "ngForTrackBy"], [1, "NgxEditor__MenuItem--Button", 3, "mousedown", "disabled"], [1, "NgxEditor__ColorContainer"], ["class", "NgxEditor__Color", 3, "ngStyle", "title", "ngClass", "mousedown", 4, "ngFor", "ngForOf", "ngForTrackBy"], [1, "NgxEditor__Color", 3, "mousedown", "ngStyle", "title", "ngClass"]],
...
var BubbleComponent = class _BubbleComponent {
  ...
  static {
    this.ɵfac = function BubbleComponent_Factory(__ngFactoryType__) {
  ...
  static {
    this.ɵcmp = ɵɵdefineComponent({
      ...
      decls: 1,
      vars: 2,
      consts: [[4, "ngFor", "ngForOf", "ngForTrackBy"], ["class", "NgxBubbleMenu__Icon", 3, "ngClass", "title", "innerHTML", "mousedown", 4, "ngIf"], ["class", "NgxBubbleMenu__Seperator", 4, "ngIf"], [1, "NgxBubbleMenu__Icon", 3, "mousedown", "ngClass", "title", "innerHTML"], [1, "NgxBubbleMenu__Seperator"]],
...
var FloatingMenuComponent = class _FloatingMenuComponent {
  ...
  static {
    this.ɵfac = function FloatingMenuComponent_Factory(__ngFactoryType__) {
...
var MenuModule = class _MenuModule {
  static {
    this.ɵfac = function MenuModule_Factory(__ngFactoryType__) {
...
var NgxEditorModule = class _NgxEditorModule {
  ...
  static {
    this.ɵfac = function NgxEditorModule_Factory(__ngFactoryType__) {
